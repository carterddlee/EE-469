...
always_comb begin
    ...
    if (current_stage == stage_mem && opcode_mem == 7'b0000011) begin
        ...
        case (funct3_mem)
            3'b000: load_data_mem = {{24{b[7]}}, b};   // LB (sign-extend) [web:5]
            3'b001: load_data_mem = {{16{h[15]}}, h};  // LH (sign-extend) [web:5]
            3'b010: load_data_mem = data_mem_rsp.data; // LW [web:3]
            3'b100: load_data_mem = {24'b0, b};        // LBU (zero-extend) [web:5]
            3'b101: load_data_mem = {16'b0, h};        // LHU (zero-extend) [web:5]
...
always_comb begin
    ...
    // Default: write back for ALU-type ops you already support
    if (opcode_wb == 7'b0110011 || opcode_wb == 7'b0010011 ||
    ...
    if (opcode_wb == 7'b0000011) begin
        wb_we    = 1'b1;
        wb_value = alu_result_wb; // overwritten in seq below with captured load value
    end

    // Stores do not write back
    if (opcode_wb == 7'b0100011) begin
        wb_we = 1'b0;
    end
end
...
// Adds: hold in MEM until data_mem_rsp.valid for loads/stores
// -------------------------
